...
const INTENT_KEYWORDS = {
  ...
  agencyOwnership: [
    ...
    "transfer this agency to me",
    "move this agency to me",
  ],
  ...
  ownershipRequest: [
    ...
    "assign this account",
    "update this account",
    "get an account transferred to me",
    "transfer an account to me",
    "move an account to me",
    "get ownership of an account",
    "how do i get an account transferred to me",
    "how do i transfer an account to me",
    "how do i get ownership of an account",
    "how do i get an account transferred to me",
    "account transferred to me",
  ],
...
function detectClarificationNeed(lower) {
  if (
    matchesAny(lower, [
      "account transferred to me",
      "transfer an account to me",
    ...
    !matchesAny(lower, ["agency", "advertiser"])
  ) {
    return {
      category: "request_change",
      question: "Is this an advertiser account or an agency account?",
    };
  }
  ...
  if (
    ...
    !matchesAny(lower, ["confirmed", "crossmedia", "sales credit / referral request"])
  ) {
    return {
      category: "simple_question",
      question: "Is this definitely a referral already, or are you still trying to confirm whether it qualifies?",
    };
  }
  ...
  if (
    ...
    return {
      ...
      question: "What action are you trying to take, and what is the exact error message?",
    };
  }
...
const SYSTEM_PROMPT = `
You are the Kargo BizOps Onboarding & Enablement Assistant.

Answer like a BizOps team member in Slack:
- fast
- actionable
- minimal explanation
- start with the answer
- use bullets
- always give the seller the next step

Your job:
1. identify the issue
2. determine who can fix it
3. tell the seller exactly what to do next

Do not invent rules, naming conventions, or workflows.
If unsure, ask a clarifying question or route to BizOps.

Return JSON only in this shape:
{
  "category": "system_error | seller_task | simple_question | request_change",
  "shouldEscalate": true,
  "needsClarification": false,
  "answer": "Slack-ready response using markdown bullets"
}

Important rules:
- Agency account ownership changes are BizOps-managed.
- Advertiser account ownership transfers can use the standard request path.
- Product setup requires Budget, Flight Start Date, and Flight End Date on every product.
- Do not tell users to manually edit schedules.
- Closed Won cannot be edited by sellers.
- Use #bizops_help when the seller cannot fix the issue, approval is needed, or the path is unclear.
`;

function buildOpenAIInput({ userText, priorState = {}, knowledge = null }) {
  return `
User message:
${userText}
...
Grounded knowledge:
${knowledge?.found ? JSON.stringify(knowledge, null, 2) : "No grounded knowledge found."}

Return only valid JSON. No markdown wrapper. No extra commentary.
`;
}
...
async function getOpenAIBizOpsReply(userText, priorState = {}, knowledge = null) {
  if (!openai) {
  ...
  const response = await openai.responses.create({
    ...
    input: buildOpenAIInput({ userText, priorState, knowledge }),
  });
  ...
  if (!parsed?.answer) {
    return {
      ...
      needsClarification: false,
      answer:
        "⚠️ *I hit an issue formatting the response.*\n\n📌 *Next step:*\nTry again, or escalate in #bizops_help with the record link and exact error.",
    };
  }
...
export function getMockBizOpsReply(text, priorState = {}) {
  ...
  const intent = detectIntent(lower);
  const clarification = detectClarificationNeed(lower);
  ...
  if (clarification) {
    return {
      ...
      answer: formatSlackReply({
        ...
        intro: "I can point you to the right path once I know one more detail.",
...
export async function getBizOpsReply(userText, priorState = {}, knowledge = null) {
  const useOpenAI =
    process.env.USE_OPENAI === "true" && process.env.OPENAI_API_KEY;

  if (useOpenAI) {
    return getOpenAIBizOpsReply(userText, priorState, knowledge);
  }

  return getMockBizOpsReply(userText, priorState);
}